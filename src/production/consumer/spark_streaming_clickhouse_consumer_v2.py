"""Spark Structured Streaming consumer for real-time crypto market monitoring (V2)."""
...
CLICKHOUSE_ALERT_TABLE = os.environ.get("CLICKHOUSE_ALERT_TABLE", "crypto_alert_events")
...
ALERT_BATCH_INTERVAL = os.environ.get("ALERT_BATCH_INTERVAL", "10 seconds")

WATERMARK_DELAY = os.environ.get("WATERMARK_DELAY", "30 seconds")
WINDOW_DURATION = os.environ.get("WINDOW_DURATION", "1 minute")

RETURN_ALERT_THRESHOLD = float(os.environ.get("RETURN_ALERT_THRESHOLD", "1.0"))
VOLUME_ALERT_THRESHOLD = float(os.environ.get("VOLUME_ALERT_THRESHOLD", "5.0"))
VOLATILITY_ALERT_THRESHOLD = float(os.environ.get("VOLATILITY_ALERT_THRESHOLD", "0.5"))
...
CHECKPOINT_DIR_ALERT = os.environ.get("CHECKPOINT_DIR_ALERT", "checkpoints/crypto_alert_events")

spark = (
    SparkSession.builder.appName("CryptoTickConsumer-Streaming-V2")
    .config(
        "spark.jars.packages",
...
print("=" * 72)
print("Spark Structured Streaming Consumer (Crypto Monitoring V2)")
...
print(f"Alerts sink        : {CLICKHOUSE_DATABASE}.{CLICKHOUSE_ALERT_TABLE}")
...
df_windowed = (
    ...
    .withColumn("volume_per_trade", when(col("trade_count") > 0, col("total_volume") / col("trade_count")).otherwise(lit(0.0)))
    ...
    .withColumn("is_volatility_breakout", col("volatility_pct") >= lit(VOLATILITY_ALERT_THRESHOLD))
    .select(
        col("symbol"),
        col("product_id"),
        ...
        spark_round(col("volume_per_trade"), 8).alias("volume_per_trade"),
        col("is_price_spike"),
        col("is_volume_anomaly"),
        col("is_volatility_breakout"),
...
def write_metrics_to_clickhouse(batch_df, batch_id):
    row_count = batch_df.count()
    if row_count == 0:
        print(f"[Metrics Batch {batch_id}] No updated windows")
        return

    print(f"[Metrics Batch {batch_id}] Writing {row_count} rows -> {CLICKHOUSE_AGG_TABLE}")
    (
        batch_df.write.jdbc(
            url=CLICKHOUSE_JDBC_URL,
...
metrics_query = (
    df_windowed.writeStream.foreachBatch(write_metrics_to_clickhouse)
    .outputMode("update")
...
df_alerts = (
    df_windowed
    .filter(col("is_price_spike") | col("is_volume_anomaly") | col("is_volatility_breakout"))
    .withColumn(
        "alert_type",
        when(col("is_price_spike"), lit("price_spike"))
        .when(col("is_volatility_breakout"), lit("volatility_breakout"))
        .otherwise(lit("volume_anomaly"))
    )
    .withColumn(
        "severity",
        when(spark_abs(col("return_pct_1m")) >= lit(2.5), lit("high"))
        .when(col("volatility_pct") >= lit(1.0), lit("high"))
        .when(col("total_volume") >= lit(10.0), lit("high"))
        .otherwise(lit("medium"))
    )
    .withColumn(
        "metric_value",
        when(col("is_price_spike"), col("return_pct_1m"))
        .when(col("is_volatility_breakout"), col("volatility_pct"))
        .otherwise(col("total_volume"))
    )
    .withColumn(
        "threshold",
        when(col("is_price_spike"), lit(RETURN_ALERT_THRESHOLD))
        .when(col("is_volatility_breakout"), lit(VOLATILITY_ALERT_THRESHOLD))
        .otherwise(lit(VOLUME_ALERT_THRESHOLD))
    )
    .withColumn("event_time", col("window_end"))
    ...
    .select(
        ...
        "alert_type",
        "severity",
        spark_round(col("metric_value"), 4).alias("metric_value"),
        spark_round(col("threshold"), 4).alias("threshold"),
        "window_start",
        "window_end",
        "created_at",
    )
...
def write_alerts_to_clickhouse(batch_df, batch_id):
    row_count = batch_df.count()
    if row_count == 0:
        print(f"[Alert Batch {batch_id}] No alerts")
        return

    print(f"[Alert Batch {batch_id}] Writing {row_count} rows -> {CLICKHOUSE_ALERT_TABLE}")
    (
        batch_df.write.jdbc(
            url=CLICKHOUSE_JDBC_URL,
            table=CLICKHOUSE_ALERT_TABLE,
            mode="append",
            properties=CLICKHOUSE_PROPERTIES,
        )
    )
...
alerts_query = (
    df_alerts.writeStream.foreachBatch(write_alerts_to_clickhouse)
    .outputMode("update")
    .trigger(processingTime=ALERT_BATCH_INTERVAL)
    .option("checkpointLocation", CHECKPOINT_DIR_ALERT)
    .queryName("crypto_alert_events_to_clickhouse")
    .start()
)

print("\nStreaming consumer is running with 3 streams:")
...
print(f"  3. Alerts      -> {CLICKHOUSE_DATABASE}.{CLICKHOUSE_ALERT_TABLE} ({ALERT_BATCH_INTERVAL})")
